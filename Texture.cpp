...
: _renderer(renderer)
{
	...
	if (surf == nullptr)
		throw std::runtime_error(
				std::string("Что-то пошло не так. Не удается загрузить текстуру: ") + std::string(SDL_GetError()));
	...
	if (_texture == nullptr)
		throw std::runtime_error(
				std::string("Что-то пошло не так. Не удается создать текстуру: ") + std::string(SDL_GetError()));
}
